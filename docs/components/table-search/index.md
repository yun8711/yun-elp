...
| 参数 | 说明 | 类型 | 默认值 |
| --- | --- | --- | --- |
| options | 字段配置 | TableSearchOption[] \| ((params: DynamicPropsParams) => TableSearchOption[]) | [] |
...
| comp | 字段组件类型 | `Component` | `ElInput` |
| value | 字段初始值 | `any` | `""` |
| hidden | 是否隐藏 | `boolean \| ((params: DynamicPropsParams) => boolean)` | `false` |
| disabled | 是否禁用 | `boolean \| ((params: DynamicPropsParams) => boolean)` | `false` |
| valueFormat | 值格式化函数，在值更新时对值进行处理，可以返回单个值或对象（对象会被合并到formatForm中） | `((value: any, prop: string, form: Record<string, any>) => any \| Record<string, any>) \| undefined` | - |
| borderAttrs | y-border-label支持的属性 | `BorderLabelProps & Record<string, any> \| ((params: DynamicPropsParams) => BorderLabelProps & Record<string, any>)` | - |
| innerAttrs | 给内部组件的属性 | `Record<string, any> \| ((params: DynamicPropsParams) => Record<string, any>)` | - |
...
| 参数 | 说明 | 类型 |
| --- | --- | --- |
| form | 当前表单的所有值 | `Record<string, any>` |
...
| change | 表单值变化时触发 | `(form: Record<string, any>)` |
| fold | 折叠状态变化时触发 | `(isFold: boolean)` |
...
| --- | --- | --- |
| right | 右侧内容 | - |
| {prop} | 自定义字段组件，{prop} 为字段名 | `{ prop: string, value: any, item: TableSearchItem, form: Record<string, any> }` |
