...
| tipProps   | 错误提示tooltip的属性 | ^[object]`Partial<ElTooltipProps>` / ^[Function]`(scope: any, prop: string) => Partial<ElTooltipProps>` | `{popperClass:'y-column-form__error-tooltip',effect:'dark',placement:'top',enterable:false}` |
...
| 名称   | 说明                                                                      | 参数                                              |
| ------ | ------------------------------------------------------------------------- | ------------------------------------------------- |
| [prop] | 具名插槽，根据 options 中每个表单项的 prop 属性命名，用于自定义表单项内容 | ^[object]`{ scope: any, row: any, prop: string }` |
