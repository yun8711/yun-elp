---
title: CronPicker 定时选择器
description: CronPicker 定时选择器
---
...
| 属性名           | 说明                                     | 类型                                          | 默认值             |
| ---------------- | ---------------------------------------- | --------------------------------------------- | ------------------ |
| modelValue       | Cron表达式值                             | ^[string]                                     | `''`               |
| disabledPeriod   | 禁止下拉周期选项                         | ^[array]`string[]`                            | `[]`               |
| editDefaultValue | 修改默认值，例如切换到年时默认选中的时间 | ^[object]`Record<string, any>`                | `{}`               |
| defaultPeriod    | 默认调度周期                             | ^[enum]`'MINUTE' \| 'HOUR' \| 'DAY' \| 'WEEK' \| 'MONTH' \| 'YEAR'` | `'MINUTE'`         |
