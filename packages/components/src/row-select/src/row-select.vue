<template>
  <div class="y-row-select" :style="{
    ...
    <!-- 左侧 label -->
    <div class="y-row-select__label" :class="{ 'y-row-select__label--separator': props.separator }"
      :style="labelStyles">
      {{ props.labelText }}
    </div>
    ...
    <div class="y-row-select__options-container">
      <div ref="optionsBoxRef" class="y-row-select__options" @click="onItemClick">
        ...
        <template v-if="props.showAll">
          <div class="y-row-select__item" :class="{ 'is-active': selectAll }" :style="optionsItemStyles"
            data-index="$all">
            <slot name="all">
              {{ props.allText }}
            </slot>
        ...
        <!-- 选项列表 -->
        <div v-for="(item, index) in options" :key="index" class="y-row-select__item"
          :class="{ 'is-active': isItemSelected(item), 'is-disabled': item[props.defineProps.disabled] }"
          :style="optionsItemStyles"
          :data-index="item[props.defineProps.disabled] ? undefined : item[props.defineProps.value]">
          <div class="y-row-select__item-text">
            <slot v-bind="item" :index="index">
              {{ item[props.defineProps.label] }}
            </slot>
          </div>
        </div>
      </div>
    ...
    <div class="y-row-select__fold">
      <div v-if="showMore" class="y-row-select__fold-inner" :style="btnStyles" @click="() => trigger()">
...
import { ref, computed, onMounted, onBeforeUnmount, watch, nextTick } from '@vue/runtime-core'
...
const btnStyles = computed(() => {
  return {
    ...props.btnStyles,
    'flex-direction': props.iconPosition === 'left' ? 'row' : 'row-reverse'
  }
})

// 选项宽度
const itemWidth = computed(() => {
  return isNumber(props.itemWidth) ? props.itemWidth + 'px' : props.itemWidth
})
// 选项高度
const itemHeight = computed(() => {
  return isNumber(props.itemHeight) ? props.itemHeight + 'px' : props.itemHeight
})
// 选项间距
const gap = computed(() => {
...
const rowGap = computed(() => {
  ...
  const numValue = typeof gapValue === 'string'
    ? parseInt(gapValue.replace('px', ''), 10) || 8
    : Number(gapValue) || 8;
  return numValue;
})
    ...
    if (!props.single) {
      ...
      if (keySet.value.size > 0) {
        ...
        v = Array.from(keySet.value).map(k => {
          const option = props.options.find((item: any) => String(item[props.defineProps.value]) === k)
...
const isItemSelected = (item: any) => {
  return !selectAll.value && (props.single ? selectedKey.value === item[props.defineProps.value] : (selectedKey.value as any[]).includes(item[props.defineProps.value]))
}
  ...
  if (!props.single) {
    if (key === '$all') {
      ...
      res = Array.from(keySet.value).map(keyStr => {
        const option = props.options.find((item: any) => String(item[props.defineProps.value]) === keyStr)
...
const minHeight = computed(() => {
  // 确保itemHeight是数字
  const itemHeightNum = typeof props.itemHeight === 'string'
...
watch(() => props.modelValue, (newValue: string | string[] | number | number[]) => {
  if (!props.single) {
    keySet.value.clear()
