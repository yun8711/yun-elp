...
function buildExampleSummary(tagName: string, examples: ComponentExample[]) {
  ...
  return examples
    .map(example => {
      const lines = [`### ${example.title || example.name}`, '', `- 名称: ${example.name}`, `- 文档: ${example.docUrl || ''}`]
  ...
  server.registerTool(
    'get_component_examples',
    {
      ...
      inputSchema: z.object({
        ...
        exampleName: z.string().optional().describe('示例名称；includeSource=true 时可指定要读取的示例'),
    ...
    async ({ tagName, exampleName, includeSource }) => {
      ...
      if (includeSource) {
        const example = exampleName ? examples.find(item => item.name === exampleName) : examples[0]
